...
function buildSessionCode(sessionId, entryTime) {
...
function getReservationIdFromBookingCode(value) {
...
async function getDefaultFee(pool, vehicleTypeId) {
...
async function getCheckoutFee(pool, sessionId) {
  const result = await pool.request()
  ...
  try {
    ...
    if (finalReservationId) {
      ...
      if (!reservation) {
        ...
        return res.status(404).json({
          success: false,
          message: "Không tìm thấy mã đặt chỗ.",
        });
      }
      ...
      if (reservation.ReservationStatus !== "Reserved") {
        ...
        return res.status(400).json({
          success: false,
          message: "Đặt chỗ này không còn hiệu lực.",
        });
      }
    ...
    if (activeDriverResult.recordset.length > 0) {
      ...
      return res.status(409).json({
        success: false,
        message: "Tài xế này đang có một phiên đỗ xe hoạt động.",
      });
    }
    ...
    if (activeSlotResult.recordset.length > 0) {
      ...
      return res.status(409).json({
        success: false,
        message: "Vị trí này đang có xe đỗ, không thể check-in.",
      });
    }
    ...
    if (Number(slot.VehicleTypeID) !== Number(finalVehicleTypeId)) {
      ...
      return res.status(400).json({
        success: false,
        message: "Loại xe không phù hợp với vị trí đỗ.",
      });
    }
    ...
    if (!isReservedBookingCheckIn && !isWalkInCheckIn) {
      ...
      return res.status(409).json({
        success: false,
        message: "Vị trí đỗ hiện không khả dụng.",
      });
    }
    ...
    const defaultFee = await getDefaultFee(pool, finalVehicleTypeId);
...
export async function checkOutVehicle(req, res, next) {
  ...
  try {
    ...
    if (!session) {
      ...
      return res.status(404).json({
        success: false,
        message: "Không tìm thấy phiên đỗ xe.",
      });
    }
    ...
    if (session.SessionStatus !== "Active") {
      ...
      return res.status(400).json({
        success: false,
        message: "Phiên đỗ xe này đã hoàn tất hoặc không còn hoạt động.",
      });
    }

    const { fee } = await getCheckoutFee(pool, sessionId);
