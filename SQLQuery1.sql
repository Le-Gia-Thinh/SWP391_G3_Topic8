...
-- =====================================================
-- BƯỚC 4: XÓA TABLES (con trước, cha sau)
-- =====================================================
...
-- ── Users: tích hợp sẵn các cột auth + email verify ──────────────
CREATE TABLE Users (
    UserID              INT IDENTITY(1,1) PRIMARY KEY,
    FullName            NVARCHAR(100) NOT NULL,
    Email               NVARCHAR(100) UNIQUE NOT NULL,
    PasswordHash        NVARCHAR(256) NULL,          -- NULL = social-only account
...
-- ── UserAuthProviders: local / google / facebook ──────────────────
...
-- ── RefreshTokens: lưu hash, không lưu raw ───────────────────────
...
-- Driver (RoleID=1) không bị ràng buộc tuổi
-- Staff/Manager (RoleID=2,3) phải >= 18 tuổi khi vào làm
-- =====================================================
ALTER TABLE Users
...
-- ── sp_RegisterLocal ─────────────────────────────────────────────
-- Tạo Driver mới, IsEmailVerified = 0, gửi token qua BE → email
-- ─────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    VALUES (
        @FullName, @Email, @PasswordHash, @PhoneNumber,
        1,  -- Driver
        1,  -- IsActive = 1 (account tồn tại, nhưng FE/BE chặn login nếu chưa verify)
        0,  -- IsEmailVerified = 0
        @EmailVerifyToken,
        @EmailVerifyExpires
    );
    ...
    -- local provider: ProviderUserID = UserID để tránh NULL unique conflict
...
-- ── sp_GetUserByEmail ─────────────────────────────────────────────
-- Dùng cho login local — trả thêm IsEmailVerified
-- ─────────────────────────────────────────────────────────────────
...
-- ── sp_VerifyEmail ────────────────────────────────────────────────
-- User click link verify → set IsEmailVerified = 1, xóa token
-- ─────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    WHERE EmailVerifyToken   = @Token
      AND EmailVerifyExpires > GETDATE()
...
-- Cập nhật token mới khi user yêu cầu gửi lại email verify
-- ─────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- Không lộ thông tin: email không tồn tại hoặc đã verified
    IF @UserID IS NULL
    BEGIN
...
--
-- Luồng:
--   1. Tìm theo (ProviderName, ProviderUserID) → đã link rồi thì lấy luôn
--   2. Tìm theo email:
--      - IsEmailVerified = 0  → RAISERROR EMAIL_NOT_VERIFIED (tránh victim attack)
--      - IsEmailVerified = 1  → link provider vào account cũ + thông báo
--   3. Không có user → tạo Driver mới, IsEmailVerified = 1 (social đã verify)
-- ─────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    DECLARE @IsNewLink       BIT = 0;  -- dùng để BE biết có cần toast "đã link" không
    ...
    -- Bước 1: Tìm theo provider (đã link trước đó)
    SELECT @UserID = UserID
    FROM UserAuthProviders
    WHERE ProviderName   = @ProviderName
      AND ProviderUserID = @ProviderUserID;

    -- Bước 2: Chưa có provider → tìm theo email
    ...
    BEGIN
        ...
        BEGIN
            -- SECURITY: email tồn tại nhưng local chưa verify → từ chối
            ...
            -- Email verified → sẽ link provider vào account cũ
            SET @IsNewLink = 1;
        END
    END

    -- Bước 3: Không có user nào → tạo Driver mới
    IF @UserID IS NULL
    BEGIN
        INSERT INTO Users (
            FullName, Email, PasswordHash,
            RoleID, AvatarUrl, IsActive,
            IsEmailVerified  -- social provider đã verify email
        )
        VALUES (
            @FullName, @Email, NULL,
            1, @AvatarUrl, 1,
    ...
    -- Bước 4: Link provider nếu chưa có
    IF NOT EXISTS (
        SELECT 1 FROM UserAuthProviders
    ...
    SELECT
        ...
        u.AvatarUrl,
        @IsNewLink AS IsNewLink  -- BE dùng để toast "Đã liên kết Google/Facebook"
    FROM Users u
    JOIN Roles r ON u.RoleID = r.RoleID
    WHERE u.UserID = @UserID;
END
...
-- ── sp_SaveRefreshToken ───────────────────────────────────────────
-- Lưu hash refresh token, dọn token cũ của user
-- ─────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- Dọn token cũ đã revoke hoặc hết hạn
    DELETE FROM RefreshTokens
    WHERE UserID = @UserID
      AND (RevokedAt IS NOT NULL OR ExpiresAt < GETDATE());
...
-- Thu hồi token khi logout hoặc rotate
-- ─────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    WHERE ps.SlotID IN (
        SELECT DISTINCT i.SlotID FROM inserted i WHERE i.SlotID IS NOT NULL
    );
END
GO

CREATE TRIGGER TRG_RecalculateSlotStatus_OnReservation
ON Reservations AFTER INSERT, UPDATE
AS
BEGIN
    SET NOCOUNT ON;
    UPDATE ps
...
-- Users mẫu: Staff/Manager IsEmailVerified=1 (seed data, không qua flow verify)
--            Driver seed cũng = 1 vì đây là data test, không phải đăng ký thật
...
-- Link local provider cho tất cả seed users
INSERT INTO UserAuthProviders (UserID, ProviderName, ProviderUserID, ProviderEmail)
...
SELECT TableName, Rows FROM (
    ...
    SELECT 'UserAuthProviders',COUNT(*) FROM UserAuthProviders                     UNION ALL
...
SELECT
    u.UserID, u.Email, u.IsActive, u.IsEmailVerified,
    uap.ProviderName
FROM Users u
...
SELECT 'SPs' AS Info, name FROM sys.procedures
WHERE name IN (
    'sp_RegisterLocal','sp_GetUserByEmail',
    'sp_VerifyEmail','sp_ResendVerifyEmail',
    'sp_UpsertSocialUser','sp_SaveRefreshToken',
    'sp_GetUserByRefreshToken','sp_RevokeRefreshToken',
    'sp_CheckInVehicle','sp_CheckOutVehicle','sp_CreateReservation'
)
ORDER BY name;
GO
...
PRINT '';
PRINT '✅ ParkingManagementDB khởi tạo hoàn tất!';
PRINT '   Tiếp theo: cập nhật authService.js (BE) → authController.js → routes → FE';
GO