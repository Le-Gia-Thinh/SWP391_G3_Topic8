          ...
          <div className="overflow-x-auto overflow-y-auto max-h-130">
            {loading ? (
              ...
              <div className="py-16 flex flex-col items-center justify-center text-center text-slate-500">
                ...
                <p className="font-bold text-slate-700">{t('admin.auditLog.emptyTitle')}</p>
                <p className="text-sm mt-1 text-slate-500">{t('admin.auditLog.emptyHint')}</p>
              </div>
            ) : (
              <table className="min-w-full text-left text-sm text-slate-700">
                <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
                  <tr>
                    <th className="px-5 py-4 font-bold text-[12px] text-slate-500 bg-slate-50">{t('admin.auditLog.col.actor')}</th>
                    <th className="px-5 py-4 font-bold text-[12px] text-slate-500 bg-slate-50">{t('admin.auditLog.col.action')}</th>
                    <th className="px-5 py-4 font-bold text-[12px] text-slate-500 bg-slate-50">{t('admin.auditLog.col.target')}</th>
                    <th className="px-5 py-4 font-bold text-[12px] text-slate-500 bg-slate-50">{t('admin.auditLog.col.detail')}</th>
                    <th className="px-5 py-4 font-bold text-[12px] text-slate-500 bg-slate-50">{t('admin.auditLog.col.time')}</th>
                  </tr>
                </thead>
