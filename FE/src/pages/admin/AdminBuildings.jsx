    ...
    <div className={`space-y-6 pb-12 transition-all duration-700 ease-out ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      ...
      <div className="rounded-3xl bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200/60">
        ...
        {loading ? (
          ...
          <div className="py-16 flex flex-col items-center justify-center text-center text-slate-500 font-medium">
            ...
            <p className="font-bold text-slate-700 font-bold">{t('admin.buildings.emptyTitle')}</p>
            <p className="text-sm mt-1 text-slate-500 font-medium">{t('admin.buildings.emptyHint')}</p>
          </div>
        ) : (
            ...
            {rows.map(b => (
              <div key={b.BuildingID} className="rounded-2xl border border-slate-200 p-5 hover:shadow-md hover:border-blue-200 transition group">
                <div className="flex items-start justify-between">
                  <div className="flex h-11 w-11 items-center justify-center rounded-3xl bg-linear-to-br from-blue-500 to-indigo-600 text-white shadow-md shadow-blue-500/20">
                    <Building2 size={22} />
                  </div>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-slate-700 font-bold dark:text-slate-200 mb-1.5">{t('admin.buildings.modal.buildingName')}</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 font-bold dark:text-slate-200 mb-1.5">{t('admin.buildings.modal.address')}</label>
            <input {...register('Address')}
              className="w-full rounded-3xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-white px-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-slate-700 font-bold dark:text-slate-200 mb-1.5">{t('admin.buildings.modal.operatingHours')}</label>
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 font-bold dark:text-slate-200 mb-1.5">{t('admin.buildings.modal.totalFloors')}</label>
