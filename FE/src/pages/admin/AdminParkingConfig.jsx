...
const AdminParkingConfig = () => {
  ...
  return (
    <div className={`space-y-6 pb-12 transition-all duration-700 ease-out ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      ...
      {buildings.length === 0 ? (
        ...
        <div className="rounded-3xl bg-white p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200/60">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex h-10 w-10 items-center justify-center rounded-3xl bg-blue-50 text-blue-600"><Building size={20} /></div>
            <div>
              <h2 className="text-lg font-bold text-slate-900">{t('admin.parkingConfig.chooseBuilding.title')}</h2>
              <p className="text-[12px] font-medium text-slate-500 font-medium mt-0.5">{t('admin.parkingConfig.chooseBuilding.hint')}</p>
            </div>
          </div>
            ...
            {buildings.map(b => {
              ...
              return (
                <button key={b.BuildingID} onClick={() => switchBuilding(b.BuildingID)}
                  className={`rounded-2xl border px-4 py-3 text-left transition ${active ? 'border-blue-400 bg-blue-50 shadow-[0_8px_30px_rgb(0,0,0,0.04)]' : 'border-slate-200 hover:border-blue-200'}`}>
                  <p className={`text-sm font-bold ${active ? 'text-blue-800' : 'text-slate-800 font-black'}`}>{b.BuildingName}</p>
                  <p className="text-[11px] text-slate-500 font-medium mt-0.5">{t('admin.parkingConfig.chooseBuilding.floorsAndSlots', { floors: b.ActualFloors ?? 0, slots: b.SlotCount ?? 0 })}</p>
                </button>
              )
            })}
          </div>
      ...
      {/* Khu vực Tầng */}
      {selectedBuilding && (
        <div className="rounded-3xl bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200/60">
          <div className="flex items-center justify-between p-5 border-b border-slate-100">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-3xl bg-indigo-50 text-indigo-600"><Layers size={20} /></div>
              <div>
                ...
                <p className="text-[12px] font-medium text-slate-500 font-medium mt-0.5">{buildingFloors.length} {t('admin.parkingConfig.floorsSection.countSuffix')}</p>
              </div>
            </div>
            <button onClick={() => setFloorModal({ mode: 'create' })}
              className="inline-flex items-center gap-2 rounded-3xl bg-linear-to-r from-blue-600 to-indigo-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-blue-500/30 hover:-translate-y-0.5 active:scale-95 transition-all">
  ...
  return (
    <section className={`rounded-2xl border ${inactive ? 'border-slate-200 bg-slate-50/50' : 'border-slate-200'}`}>
      <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <div className={`flex h-9 w-9 items-center justify-center rounded-xl ${inactive ? 'bg-slate-200 text-slate-500 font-medium' : 'bg-indigo-50 text-indigo-600'}`}>
            <Layers size={18} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-slate-900">{floor.FloorName}</h3>
              <span className={`inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[11px] font-semibold border ${floor.IsActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200/60' : 'bg-slate-100 text-slate-500 font-medium border-slate-200'}`}>
            ...
            <p className="text-[11px] text-slate-500 font-medium mt-0.5">{t('admin.parkingConfig.floor.zoneAndSlotCount', { zones: floor.ZoneCount ?? zones.length, slots: floor.SlotCount ?? 0 })}</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={onToggle} title={floor.IsActive ? t('admin.parkingConfig.floor.toggleOffTitle') : t('admin.parkingConfig.floor.toggleOnTitle')}
            className={`inline-flex items-center gap-1.5 rounded-xl border px-2.5 py-1.5 text-xs font-semibold transition ${floor.IsActive ? 'border-slate-200 text-slate-600 hover:bg-slate-50' : 'border-emerald-200 text-emerald-600 hover:bg-emerald-50'}`}>
            <Power size={13} /> {floor.IsActive ? t('admin.parkingConfig.floor.toggleOff') : t('admin.parkingConfig.floor.toggleOn')}
          </button>
          <button onClick={onEdit} title={t('admin.parkingConfig.floor.editTitle')} className="rounded-xl p-2 text-slate-500 font-medium hover:bg-blue-50 hover:text-blue-600 transition"><Pencil size={15} /></button>
          <button onClick={onDelete} title={t('admin.parkingConfig.floor.deleteTitle')} className="rounded-xl p-2 text-slate-500 font-medium hover:bg-rose-50 hover:text-rose-600 transition"><Trash2 size={15} /></button>
        </div>
      </div>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={onAddZone}
            className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-700 font-bold hover:bg-slate-50 hover:border-blue-200 transition">
...
const ZoneCard = ({ zone, onManage, onEdit, onDelete }) => {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 p-4 hover:border-blue-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition group">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-50 text-slate-500 font-medium"><Map size={16} /></div>
          <div>
            <p className="text-sm font-bold text-slate-900">{zone.ZoneName}</p>
            <p className="text-[11px] text-slate-500 font-medium flex items-center gap-1 mt-0.5"><Car size={11} /> {zone.AllowedVehicleName}</p>
          </div>
        </div>
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition">
          <button onClick={onEdit} title={t('admin.parkingConfig.zone.editTitle')} className="rounded-xl p-1.5 text-slate-500 font-medium hover:bg-blue-50 hover:text-blue-600 transition"><Pencil size={14} /></button>
          <button onClick={onDelete} title={t('admin.parkingConfig.zone.deleteTitle')} className="rounded-xl p-1.5 text-slate-500 font-medium hover:bg-rose-50 hover:text-rose-600 transition"><Trash2 size={14} /></button>
        </div>
      </div>
      ...
      <button onClick={onManage}
        className="mt-3 w-full inline-flex items-center justify-center gap-1.5 rounded-xl bg-slate-50 border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 font-bold hover:bg-blue-50 hover:text-blue-700 hover:border-blue-200 transition">
  ...
  return (
    <Modal isOpen onClose={onClose}
      ...
      <form className="space-y-4">
        <div>
          <label className="block text-sm font-semibold text-slate-700 font-bold dark:text-slate-200 mb-1.5">{t('admin.parkingConfig.floorModal.label')}</label>
...
const ZoneModal = ({ mode, data, floorId, onClose, onSaved }) => {
  ...
  return (
    <Modal isOpen onClose={onClose}
      ...
      <form className="space-y-4">
        <div>
          <label className="block text-sm font-semibold text-slate-700 font-bold dark:text-slate-200 mb-1.5">{t('admin.parkingConfig.zoneModal.nameLabel')}</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-semibold text-slate-700 font-bold dark:text-slate-200 mb-1.5">{t('admin.parkingConfig.zoneModal.vehicleTypeLabel')}</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 font-bold dark:text-slate-200 mb-1.5">{t('admin.parkingConfig.zoneModal.totalSlotsLabel')}</label>
          ...
          {[
            ...
            return (
              <button key={tt.k} onClick={() => setTab(tt.k)}
                className={`inline-flex items-center gap-1.5 rounded-t-xl px-4 py-2.5 text-sm font-semibold border-b-2 transition ${on ? 'border-blue-600 text-blue-700' : 'border-transparent text-slate-500 font-medium hover:text-slate-700 font-bold'}`}>
        ...
        <div className="overflow-y-auto flex-1 p-6">
                    ...
                    return (
                      <div key={slot.SlotID} className={`rounded-2xl border p-3 ${busyId === slot.SlotID ? 'opacity-60' : ''} ${cfg.chip}`}>
                        ...
                        <div className="mt-2 flex flex-wrap gap-1">
                          {locked ? (
                            <span className="text-[10px] text-slate-500 font-medium">{t('admin.parkingConfig.slot.panel.lockedNotice')}</span>
                          ) : (
                            <>
                              {slot.SlotStatus !== 'Available' && (
                                <button disabled={busyId === slot.SlotID} onClick={() => cycleStatus(slot, 'Available')}
                                  className="inline-flex items-center gap-1 rounded-xl bg-white/70 border border-emerald-200 px-2 py-1 text-[10px] font-bold text-emerald-700 hover:bg-emerald-50 transition disabled:opacity-50"><Check size={11} /> {t('admin.parkingConfig.slot.panel.markAvailable')}</button>
                              )}
                              {slot.SlotStatus !== 'Maintenance' && (
                                <button disabled={busyId === slot.SlotID} onClick={() => cycleStatus(slot, 'Maintenance')}
                                  className="inline-flex items-center gap-1 rounded-xl bg-white/70 border border-amber-200 px-2 py-1 text-[10px] font-bold text-amber-700 hover:bg-amber-50 transition disabled:opacity-50">{t('admin.parkingConfig.slot.panel.markMaintenance')}</button>
                              )}
                              {slot.SlotStatus !== 'Blocked' && (
                                <button disabled={busyId === slot.SlotID} onClick={() => cycleStatus(slot, 'Blocked')}
                                  className="inline-flex items-center gap-1 rounded-xl bg-white/70 border border-red-200 px-2 py-1 text-[10px] font-bold text-red-700 hover:bg-red-50 transition disabled:opacity-50">{t('admin.parkingConfig.slot.panel.markBlocked')}</button>
                              )}
                              <button disabled={busyId === slot.SlotID} onClick={() => removeSlot(slot)} title={t('admin.parkingConfig.slot.panel.deleteSlotTitle')}
                                className="inline-flex items-center gap-1 rounded-xl bg-white/70 border border-rose-200 px-2 py-1 text-[10px] font-bold text-rose-700 hover:bg-rose-50 transition disabled:opacity-50"><Trash2 size={11} /></button>
                            </>
                          )}
                        </div>
                      </div>
          ...
          {tab === 'add' && (
            <div className="max-w-md">
              {full && (
                <div className="rounded-3xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700 mb-4 flex items-start gap-2">
              ...
              <form className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 font-bold mb-1.5">{t('admin.parkingConfig.slot.panel.addCodeLabel')}</label>
                  <input {...addForm.register('slotCode', { required: t('admin.parkingConfig.slot.panel.addCodeRequired') })} placeholder={t('admin.parkingConfig.slot.panel.addCodePlaceholder')}
                    className="w-full rounded-3xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition uppercase" />
          ...
          {tab === 'bulk' && (
            <div className="max-w-lg">
              ...
              <form className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 font-bold mb-1.5">{t('admin.parkingConfig.slot.panel.bulkPrefixLabel')}</label>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 font-bold mb-1.5">{t('admin.parkingConfig.slot.panel.bulkStartLabel')}</label>
                    <input type="number" min="0" {...bulkForm.register('start')} className="w-full rounded-3xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 font-bold mb-1.5">{t('admin.parkingConfig.slot.panel.bulkEndLabel')}</label>
                    <input type="number" min="0" {...bulkForm.register('end')} className="w-full rounded-3xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 font-bold mb-1.5">{t('admin.parkingConfig.slot.panel.bulkPadLabel')}</label>
                    <input type="number" min="1" max="4" {...bulkForm.register('pad')} className="w-full rounded-3xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition" />
                  </div>
                </div>
...
const BulkPreview = ({ bulkForm }) => {
  ...
  return (
    <div className="rounded-3xl bg-slate-50 border border-slate-200 px-4 py-3">
      ...
      <p className="text-[12px] text-slate-500 font-medium mt-1 font-mono">{sample.join(', ')}{tail}</p>
    </div>
  )
}
