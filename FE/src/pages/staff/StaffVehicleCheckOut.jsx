...
const StaffVehicleCheckOut = () => {
  ...
  const sessions = allSessions.filter(s => 
    ...
    s.booking.toLowerCase().includes(query.toLowerCase())
  )
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="flex gap-4 items-end">
              <div className="flex-1">
                <label className="block text-xs font-semibold text-gray-700 mb-1">Tìm kiếm chung</label>
                <input 
                  type="text" 
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder="Nhập biển số, mã phiên, mã thẻ..." 
                  className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm font-bold" 
                />
              </div>
              <div className="flex gap-2">
                <button onClick={() => setQuery('')} className="px-4 py-3 border border-gray-300 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-50">Làm mới</button>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                Danh sách Phiên đang hoạt động <span className="bg-blue-100 text-blue-600 text-xs px-2 py-0.5 rounded-full">{sessions.length} phiên</span>
              </h3>
            ...
            {sessions.length > 0 ? (
              ...
              <div className="border border-dashed border-gray-300 rounded-xl p-8 flex flex-col items-center text-center bg-gray-50">
                ...
                <div className="flex gap-3 justify-center">
                  <button onClick={() => setQuery('')} className="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-semibold flex items-center gap-2 hover:bg-gray-50">
