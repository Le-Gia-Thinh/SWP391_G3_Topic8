    ...
    <div className={`bg-white border rounded-xl shadow-sm hover:shadow-md transition-all p-4 flex flex-col gap-3 ${isOverdue ? 'border-orange-200 hover:border-orange-400' : 'border-gray-100 hover:border-blue-200'}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          {isPrepaid && <span className="px-2 py-0.5 rounded-full text-xs font-bold shrink-0 bg-green-100 text-green-700">Đã trả QR</span>}
      ...
      <div className={`rounded-lg px-3 py-2 flex items-center justify-between ${isPrepaid ? 'bg-green-50' : 'bg-blue-50'}`}>
        <span className={`text-xs font-semibold ${isPrepaid ? 'text-green-700' : 'text-blue-600'}`}>{isPrepaid ? 'Đã trả trước' : 'Phí ước tính'}</span>
  ...
  return (
    <div className="flex flex-col min-h-full bg-gray-50 pb-12">
      <div className="mb-2 text-sm text-gray-500 flex items-center gap-2">
        <span>Nhân viên</span><ChevronRight size={14} /><span>Thanh toán</span><ChevronRight size={14} />
      ...
      <header className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100 text-sm font-medium">
          Đồng bộ: Hoàn tất (Online) <span className="w-2 h-2 rounded-full bg-green-500 ml-1 animate-pulse" />
        </div>
      </header>
      ...
      <div className="flex gap-6">
        <div className="flex-3 flex flex-col gap-5">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <h3 className="text-base font-bold text-gray-800 flex items-center gap-2 mb-1"><FileSearch size={18} className="text-blue-600" /> Tìm kiếm phiên gửi xe</h3>
            <p className="text-xs text-gray-500 mb-3">
              Tìm theo: <span className="font-semibold text-gray-700">Mã phiên (SS-XXXXX)</span> · <span className="font-semibold text-gray-700">Mã booking (BK-XXXX)</span> · <span className="font-semibold text-gray-700">Biển số</span> · <span className="font-semibold text-gray-700">Tên / SĐT khách</span>
            </p>
            ...
            <div className="flex gap-3 mb-3">
              ...
              <button onClick={handleSearch} disabled={loading}
                ...
                {loading ? <Loader2 size={15} className="animate-spin" /> : <Search size={15} />} Tìm kiếm
              </button>
            ...
            <div className="flex items-center gap-3">
              ...
              {(fromDate || toDate) && <button onClick={() => { setFromDate(''); setToDate('') }} className="text-xs text-red-500 hover:text-red-700 font-semibold flex items-center gap-1"><X size={11} /> Xoá</button>}
            </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {searched && <span className="bg-blue-100 text-blue-700 text-xs font-bold px-2.5 py-0.5 rounded-full">{displayed.length} phiên</span>}
              {prepaidCount > 0 && <span className="bg-green-100 text-green-700 text-xs font-bold px-2.5 py-0.5 rounded-full">{prepaidCount} đã trả QR</span>}
            </div>
            {displayed.length > 0 && totalFee > 0 && (
              <span className="text-xs text-gray-500">Tổng phí ghi nhận: <span className="font-black text-gray-800">{formatVND(totalFee)}</span></span>
            )}
          </div>
            ...
            {loading ? (
              ...
              <div className="border border-dashed border-gray-300 rounded-xl p-10 flex flex-col items-center text-center bg-gray-50">
                ...
                <h4 className="text-base font-bold text-gray-800 mb-2">
                  {query || fromDate || toDate ? 'Không tìm thấy phiên nào khớp với bộ lọc' : 'Không có phiên đỗ xe nào đang hoạt động'}
                </h4>
                <p className="text-sm text-gray-500 max-w-md mb-5">Kiểm tra lại Biển số xe / Mã Phiên / Mã Booking. Nếu xe đang ở cổng nhưng không có trong hệ thống, hãy tạo Báo cáo Sự cố.</p>
                <div className="flex gap-3">
                  <button onClick={handleReset} className="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-semibold flex items-center gap-2 hover:bg-gray-50"><RefreshCcw size={15} /> Thử lại</button>
                  <button onClick={() => navigate('/staff/create-incident')} className="px-4 py-2 bg-red-50 border border-red-200 text-red-600 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-red-100"><AlertTriangle size={15} /> Tạo Báo cáo sự cố</button>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="bg-blue-50 rounded-xl border border-blue-100 p-5">
            <h3 className="text-sm font-bold text-blue-900 mb-4 flex items-center gap-2"><ChevronRight size={16} className="text-blue-500" /> Hướng dẫn xử lý</h3>
            <ul className="space-y-4 text-sm text-blue-800">
              {['Tìm bằng Biển số, Mã phiên (SS-XXXXX) hoặc Mã booking (BK-XXXX).', 'Luôn đối chiếu biển số thực tế với hệ thống trước khi tính tiền.', 'Phiên "Đã trả QR" — chỉ cần xác nhận biển số, hệ thống tự tính phụ trội nếu có.', 'Phí cuối cùng do server tính khi bấm Hoàn tất — không cần tính tay.'].map((txt, i) => (
            ...
            <div className="mt-5 bg-white rounded-lg p-4 border border-red-200 flex items-start gap-3 shadow-sm">
              ...
              <div>
                ...
                <p className="text-[11px] text-gray-600 leading-relaxed">Khách bị mất thẻ hoặc biển số mờ/thay đổi lúc check-in. Hỏi lại hoặc tạo báo cáo sự cố.</p>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-0 left-64 right-0 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
        <div className="flex items-center justify-between px-6 py-2.5 text-xs text-gray-500 font-medium">
          <div className="flex items-center gap-6">
            <span className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-green-500" /> reading: Smart_Cam_01</span>
