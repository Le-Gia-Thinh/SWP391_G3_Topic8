...
// Normalize keyword — "SS-01005" → "1005", "SESS-20260611-0003" → "3", "BK-0102" → "0102"
const normalizeKeyword = (kw) => {
  const t = kw.trim()
  // SESS-YYYYMMDD-XXXX  hoặc SS-XXXXX → lấy số cuối
  const mSess = t.match(/^(?:SESS-\d{8}-|SS-?)(\d+)$/i)
  if (mSess) return String(parseInt(mSess[1], 10)) // "0003" → "3"
  // BK-XXXX → lấy số
  const mBK = t.match(/^BK-?(\d+)$/i)
  if (mBK) return String(parseInt(mBK[1], 10))
  return t
}

// ── API calls — dùng sessionService routes ────────────────────
// GET /api/sessions  → sessionService.getSessions (trả toàn bộ)
// GET /api/sessions?status=Active  → lọc Active
// Phí do BE tính khi checkout, FE chỉ hiển thị Amount từ Payments

const fetchActiveSessions = async () => {
  const res = await axios.get('/sessions')
  const all = res.data?.data || []
  // Lọc Active ở client vì sessionService.getSessions không nhận filter
  return all.filter(s => s.SessionStatus === 'Active')
}

const searchActiveSessions = async (keyword) => {
  const res = await axios.get('/sessions')
  const all = (res.data?.data || []).filter(s => s.SessionStatus === 'Active')
...
function sortSessions(list, key) {
  ...
  switch (key) {
    ...
    case 'duration_desc': return arr.sort((a, b) => new Date(a.EntryTime) - new Date(b.EntryTime))
    case 'duration_asc': return arr.sort((a, b) => new Date(b.EntryTime) - new Date(a.EntryTime))
...
const SessionCard = ({ session, onCheckout }) => {
  ...
  const isOverdue = durationMin >= 180  // >3h highlight

  // Phí BE đã tính sẵn trong Payments.Amount khi check-in
  const prepaidAmount = Number(session.Amount || 0)
    ...
    <div className={`bg-white border rounded-xl shadow-sm hover:shadow-md transition-all p-4 flex flex-col gap-3 ${isOverdue ? 'border-orange-200 hover:border-orange-400' : 'border-gray-100 hover:border-blue-200'
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          <span className={`px-2 py-0.5 rounded-full text-xs font-bold shrink-0 ${isBooking ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'
            }`}>{isBooking ? 'Booking' : 'Vãng lai'}</span>
          {isBooking && (
            <span className="text-xs text-purple-500 font-semibold">{session.BookingCode}</span>
          )}
        </div>
      ...
      <div className="bg-blue-50 rounded-lg px-3 py-2 flex items-center justify-between">
        <span className="text-xs text-blue-600 font-semibold">
          Phí hiện tại (BE tính)
        </span>
        <span className="font-black text-blue-800 text-sm">
          {prepaidAmount > 0 ? formatVND(prepaidAmount) : '—'}
        </span>
      </div>
...
const StaffVehicleCheckOut = () => {
  ...
  const [allSessions, setAllSessions] = useState([])  // cache full list
  ...
  // Load tất cả phiên Active từ sessionService
  ...
  // Tìm kiếm: normalize → search trong cache
  const handleSearch = async () => {
    const raw = query.trim()
    if (!raw) return loadActive()

    const normalized = normalizeKeyword(raw)
    setLoading(true)
    try {
      const data = await searchActiveSessions(normalized)
      setSessions(data)
      setSearched(true)
      if (data.length === 0) toast.info('Không tìm thấy phiên nào khớp với từ khóa')
    } catch {
        ...
        <div className="flex-[3] flex flex-col gap-5 min-h-0">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <p className="text-xs text-gray-500 mb-3">
              Tìm theo:{' '}
              <span className="font-semibold text-gray-700">Mã phiên (SS-XXXXX / SESS-...)</span> ·{' '}
