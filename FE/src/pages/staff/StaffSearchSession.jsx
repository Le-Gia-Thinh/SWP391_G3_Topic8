...
function normalizeRow(s) {
  ...
  return {
    ...
    plate: s.PlateNumber || '—',
    status,
    type: s.BookingCode ? 'Booking' : 'Walk-in',
                      ...
                      <div className={`font-bold ${session.checkOut ? 'text-gray-700' : 'text-green-600'}`}>
                        {session.checkOut ? session.checkOut : (
                          <span className="flex items-center gap-1">
                            ...
                            {session.status === 'reserved' ? t('staff.searchSession.card.notEntered') : t('staff.searchSession.status.active')}
                          </span>
                        )}
                      </div>
                    </div>
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
      <div className="bg-blue-600 p-6 text-center relative overflow-hidden">
        ...
        <h3 className="text-[10px] font-bold text-blue-200 uppercase tracking-[0.2em] mb-2 relative z-10">{t('staff.searchSession.detail.header')}</h3>
