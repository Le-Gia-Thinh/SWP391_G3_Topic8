  ...
  return {
    ...
    checkIn: s.EntryTime
      ...
      : s.StartTime
        ...
        : '',
    checkOut: s.ExitTime
      ? new Date(s.ExitTime).toLocaleString('vi-VN')
...
const StaffSearchSession = () => {
  ...
  // ── Fetch từ backend mỗi khi status thay đổi ──
  const fetchSessions = useCallback(async (currentFilters) => {
    setLoading(true)
    try {
      const params = {
  ...
  }, [filters.status]) // refetch khi đổi status

  const vehicleOptions = useMemo(
    () => [...new Set(sessions.map(s => s.vehicle).filter(Boolean))],
    [sessions]
  )
  const gateOptions = useMemo(
    () => [...new Set(sessions.map(s => s.gate).filter(Boolean))],
    [sessions]
  )
  ...
  // Client-side filter cho keyword, type, vehicle, gate, date
  const filtered = useMemo(() => {
    const q = safeLower(query)
    return sessions.filter(s => {
      if (
        q &&
        !safeLower(s.plate).includes(q) &&
        !safeLower(s.id).includes(q) &&
        !safeLower(s.sessionCode).includes(q) &&
        !safeLower(s.bookingCode).includes(q) &&
        !safeLower(s.staff).includes(q)
      ) return false
      ...
      if (filters.date && s._rawCheckIn) {
        const sessionDate = s._rawCheckIn.slice(0, 10)
  ...
  const selectSession = session => {
    setSelectedSession(prev => (prev?._key === session._key ? null : session))
  }
    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-3 mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={clearFilters} className="text-xs text-blue-600 hover:text-blue-800 underline whitespace-nowrap">
            Xóa bộ lọc
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-6 flex-1 min-h-0">
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-4 min-w-0">
                      <div className={`flex-shrink-0 p-3 rounded-2xl ${isSelected ? 'bg-blue-600 text-white shadow-md' : 'bg-blue-50 text-blue-600'}`}>
                        <Car size={24} />
                      </div>
                      <div className="min-w-0">
                        <div className="text-xl font-black text-gray-900 tracking-wide leading-none whitespace-nowrap">
                          {session.plate}
                        </div>
                        <div className="text-xs font-semibold text-gray-400 mt-1.5 flex items-center gap-1.5 flex-wrap">
                          {session.sessionCode && (
                            <span className="bg-gray-100 px-1.5 py-0.5 rounded text-gray-500">{session.sessionCode}</span>
                          )}
                          {session.bookingCode && (
                            <span className="bg-blue-50 px-1.5 py-0.5 rounded text-blue-500">{session.bookingCode}</span>
                          )}
                          {session.vehicle && (
                            <>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                      <span className={`text-xs font-bold px-3 py-1 rounded-lg border ${session.type === 'Booking' ? 'border-blue-200 text-blue-700 bg-blue-50' : 'border-gray-200 text-gray-600'}`}>
                        {session.type}
                      </span>
                  ...
                  <div className="grid grid-cols-4 gap-4 text-sm bg-gray-50 rounded-xl p-3 border border-gray-100">
                    <div>
                      ...
                      <div className="font-black text-blue-600">{session.gate || '—'}</div>
                    </div>
                    <div>
                      <div className="text-gray-400 text-[10px] font-bold uppercase tracking-wider mb-1">Tài xế</div>
                      <div className="font-bold text-gray-700 truncate">{session.staff || '—'}</div>
                    </div>
                    <div>
        ...
        {/* ── Right: Detail Panel ── */}
        <div className="w-80 flex flex-col gap-4 pb-4 flex-shrink-0">
          {selectedSession ? (
            ...
            <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-8 text-center h-full flex flex-col justify-center items-center">
              ...
              <p className="text-xs text-gray-400 leading-relaxed">
                Hãy click chọn một phiên gửi xe ở danh sách bên trái để xem thông tin chi tiết.
              </p>
            </div>
          )}
        </div>
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
      <div className="bg-blue-600 p-6 text-center relative overflow-hidden">
        ...
        <h3 className="text-[10px] font-bold text-blue-200 uppercase tracking-[0.2em] mb-2 relative z-10">
          Chi tiết phiên gửi xe
        </h3>
        <p className="text-2xl font-black text-white tracking-widest relative z-10 drop-shadow-md break-all leading-tight px-2" title={session.plate}>
          {session.plate}
        </p>
        {session.vehicle && (
          <p className="text-sm font-medium text-blue-100 mt-1 relative z-10">{session.vehicle}</p>
        )}
      </div>

      <div className="p-5 overflow-auto">
        <div className={`flex justify-center items-center gap-2 py-2.5 px-4 rounded-xl text-sm font-bold mb-5 border ${stCfg.color}`}>
          {stCfg.icon} {stCfg.label}
        </div>
        ...
        <div className="space-y-4">
          <div className="space-y-2 border-b border-gray-100 pb-4">
            ...
            {session.bookingCode && (
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                  <Tag size={10} /> Mã booking
                </span>
                <span className="text-xs font-semibold text-blue-700 bg-blue-50 px-2 py-0.5 rounded font-mono">
                  {session.bookingCode}
                </span>
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-4 border-b border-gray-100 pb-4">
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Loại</p>
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-black text-blue-600">{session.gate || '—'}</p>
            </div>
          </div>
