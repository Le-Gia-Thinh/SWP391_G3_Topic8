...
const printInvoice = (session) => {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/>
    ...
    <div class="center">
      <div class="bold">PBMS - BÃI ĐỖ XE THÔNG MINH</div>
      <div class="title">HÓA ĐƠN GỬI XE</div>
      <div>${new Date().toLocaleString('vi-VN')}</div>
    </div>
    <div class="divider"></div>
    <div class="row"><span>Mã phiên:</span><span class="bold">${session.SessionCode || '—'}</span></div>
    <div class="row"><span>Biển số:</span><span class="bold">${session.PlateNumber || '—'}</span></div>
    <div class="row"><span>Loại xe:</span><span>${session.VehicleName || '—'}</span></div>
    <div class="row"><span>Vị trí:</span><span>${session.SlotCode || '—'}</span></div>
    <div class="row"><span>Vào lúc:</span><span>${formatDateTime(session.EntryTime)}</span></div>
    <div class="row"><span>Ra lúc:</span><span>${formatDateTime(session.ExitTime)}</span></div>
    <div class="row"><span>Thời gian:</span><span>${formatDuration(session.EntryTime, session.ExitTime)}</span></div>
    <div class="divider"></div>
    ${Number(session.PrepaidAmount) > 0 ? `
      <div class="row"><span>Đã trả trước (QR):</span><span>${formatVND(session.PrepaidAmount)}</span></div>
      <div class="row surcharge"><span>Phụ trội:</span><span>${formatVND(session.SurchargeAmount)}</span></div>
    ` : ''}
    <div class="row total"><span>TỔNG CỘNG:</span><span>${formatVND(session.FinalAmount || session.Amount)}</span></div>
    <div class="row"><span>Phương thức:</span><span>${session.PaymentMethod || '—'}</span></div>
    <div class="divider"></div>
    <div class="center footer"><div>Cảm ơn quý khách!</div><div>Hotline: 1900 1234</div></div>
    </body></html>`
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
      <div className="bg-orange-500 p-5 text-center">
        ...
        <p className="text-white font-black text-lg">Xác nhận thu tiền mặt</p>
      </div>
      <div className="p-6 space-y-4">
        <div className="bg-orange-50 border border-orange-100 rounded-xl p-4 space-y-2 text-sm">
          ...
          <div className="flex justify-between border-t border-orange-200 pt-2">
            <span className="font-bold text-gray-700">Cần thu thêm:</span>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex gap-2 text-xs text-yellow-800">
          ...
          <span>Nhân viên xác nhận đã thu đủ <strong>{formatVND(session.SurchargeAmount)}</strong> tiền mặt từ khách hàng.</span>
        </div>
...
const BankingConfirmDialog = ({ session, onConfirm, onCancel, onSwitchCash, loading }) => {
  ...
  // Dùng ref để đọc session mới nhất mà không cần thêm vào deps
  const sessionRef = useRef(session)
  useEffect(() => { sessionRef.current = session }, [session])

  // ✅ inline effect + cancelled flag, deps rỗng → chỉ mount 1 lần, poll tự lo phần còn lại
  useEffect(() => {
    let cancelled = false
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div className={`p-5 text-center ${hasTransfer ? 'bg-blue-600' : 'bg-gray-700'}`}>
          ...
          <p className="text-white font-black text-lg">Xác nhận chuyển khoản</p>
          <p className="text-white/70 text-xs mt-1">
            {hasTransfer ? 'Đã nhận được xác nhận từ khách' : 'Đang chờ xác nhận từ khách'}
          </p>
        </div>
        <div className="p-6 space-y-4">
          {checking ? (
            ...
            <>
              <div className="bg-green-50 border border-green-200 rounded-xl p-4 space-y-2 text-sm">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-bold text-green-700">Đã nhận xác nhận từ khách hàng</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Mã phiên:</span>
                  <span className="font-bold font-mono">{session.SessionCode}</span>
                ...
                <div className="flex justify-between border-t border-green-200 pt-2">
                  <span className="font-bold text-gray-700">Phụ trội cần thu:</span>
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 flex gap-2 text-xs text-blue-800">
                <Info size={14} className="shrink-0 mt-0.5" />
                <span>Xác nhận rằng khách đã chuyển khoản phần phụ trội <strong>{formatVND(paymentInfo?.surchargeAmount)}</strong>.</span>
              </div>
              <div className="flex gap-3">
                <button onClick={onCancel} disabled={loading}
                  className="flex-1 py-2.5 border border-gray-300 text-gray-700 font-bold rounded-xl hover:bg-gray-50 text-sm">
            ...
            <>
              <div className="bg-red-50 border border-red-200 rounded-xl p-4 space-y-2">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-bold text-red-700 text-sm">Chưa nhận được xác nhận từ khách</span>
                </div>
                <p className="text-xs text-red-600">
                  Hệ thống chưa ghi nhận giao dịch chuyển khoản từ khách hàng cho khoản phụ trội <strong>{formatVND(session.SurchargeAmount)}</strong>.
                </p>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm space-y-2">
                <p className="font-bold text-gray-700 text-xs uppercase tracking-wider mb-2">Thông tin phiên</p>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex gap-2 text-xs text-yellow-800">
                ...
                <span>Yêu cầu khách chuyển khoản phụ trội hoặc chuyển sang <strong>thu tiền mặt</strong>. Đang tự động kiểm tra lại mỗi 5 giây.</span>
              </div>
              <div className="flex items-center justify-center gap-2 text-xs text-gray-400">
                <Loader2 size={11} className="animate-spin" />
                Đang chờ xác nhận từ khách...
              </div>
              <div className="flex gap-3">
...
const SurchargeCard = ({ session, onConfirmCash, onConfirmBanking, onPrint }) => (
  <div className="bg-white border border-orange-200 rounded-xl shadow-sm p-5 flex flex-col gap-3 hover:shadow-md transition-all">
      ...
      <div>
        ...
        <div className="mt-1">
          <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-orange-100 text-orange-700 border border-orange-200 inline-flex items-center gap-1">
            <AlertCircle size={10} /> Chờ thu phụ trội
          </span>
        </div>
      </div>
      <div className="text-right">
        <p className="text-xs text-gray-400">Cần thu thêm</p>
    ...
    <div className="bg-orange-50 rounded-lg p-3 space-y-1.5 text-xs">
      <div className="flex justify-between">
        <span className="text-gray-400">Đã trả trước (QR):</span>
        <span className="font-bold text-green-600">{formatVND(session.PrepaidAmount)}</span>
      </div>
      <div className="flex justify-between">
        <span className="text-gray-400">Phí thực tế:</span>
      ...
      <div className="flex justify-between border-t border-orange-200 pt-1.5">
        <span className="font-bold text-gray-600">Phụ trội:</span>
        <span className="font-black text-orange-600">{formatVND(session.SurchargeAmount)}</span>
      </div>
    </div>
    ...
    <div className="flex gap-2 mt-1">
      ...
      <button onClick={() => onConfirmBanking(session)}
        ...
        <QrCode size={13} /> Chuyển khoản
      </button>
      <button onClick={() => onPrint(session)}
        className="px-3 py-2 border border-gray-200 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-50 transition-colors" title="In hóa đơn">
        <Printer size={13} />
      </button>
    </div>
  </div>
...
const PaymentBadge = ({ status, surchargeStatus }) => {
  if (surchargeStatus === 'Pending') return (
    <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-orange-100 text-orange-700 border border-orange-200 inline-flex items-center gap-1">
      <AlertCircle size={10} /> Chờ phụ trội
    </span>
  )
  const map = {
    Completed: { label: 'Đã thanh toán', cls: 'bg-green-100 text-green-700 border-green-200' },
    Prepaid:   { label: 'Đã trả QR', cls: 'bg-blue-100 text-blue-700 border-blue-200' },
    Pending:   { label: 'Chưa thanh toán', cls: 'bg-gray-100 text-gray-600 border-gray-200' },
    Failed:    { label: 'Thất bại', cls: 'bg-red-100 text-red-600 border-red-200' },
...
const HistoryRow = ({ session, onPrint }) => (
  <tr className="hover:bg-gray-50/50 transition-colors">
    ...
    <td className="py-3 px-4">
      ...
      {Number(session.PrepaidAmount) > 0 && (
        <p className="text-xs text-green-600">QR: {formatVND(session.PrepaidAmount)}</p>
      )}
      {Number(session.SurchargeAmount) > 0 && (
        <p className="text-xs text-orange-600">Phụ trội: {formatVND(session.SurchargeAmount)}</p>
      )}
    </td>
    <td className="py-3 px-4">
    ...
    <td className="py-3 px-4">
      <button onClick={() => onPrint(session)}
        className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="In hóa đơn">
        <Printer size={15} />
      </button>
    </td>
  </tr>
...
const SurchargeTab = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cancelled) toast.error('Không tải được danh sách phụ trội')
      } finally {
  ...
  const handleConfirmCash = async () => {
    ...
    try {
      ...
      toast.success(`Thu phụ trội tiền mặt ${formatVND(cashDialog.SurchargeAmount)} thành công!`)
      setCashDialog(null)
      reload()
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Thu phụ trội thất bại')
    } finally {
      setConfirming(false)
    }
  }
  ...
  const handleConfirmBanking = async () => {
    ...
    try {
      ...
      toast.success(`Xác nhận chuyển khoản phụ trội ${formatVND(bankDialog.SurchargeAmount)} thành công!`)
      setBankDialog(null)
      reload()
    } catch (err) {
      ...
      <div className="space-y-5">
        {sessions.length > 0 && (
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-orange-50 border border-orange-100 rounded-xl p-4 text-center">
              ...
              <p className="text-xs text-orange-700 font-semibold mt-1">Phiên chờ thu</p>
            </div>
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-center">
              <p className="text-xl font-black text-blue-600">{formatVND(totalSurcharge)}</p>
              <p className="text-xs text-blue-700 font-semibold mt-1">Tổng cần thu</p>
            </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-dashed border-gray-200 p-16 text-center">
            ...
            <p className="text-base font-bold text-gray-700">Không có phiên nào chờ thu phụ trội</p>
            <p className="text-sm text-gray-400 mt-1">Tất cả phiên đã được thanh toán đầy đủ.</p>
...
const HistoryTab = () => {
  ...
  // ✅ inline effect, đọc filter qua ref để tránh eslint warning
  useEffect(() => {
    let cancelled = false
    const run = async () => {
      setLoading(true)
      try {
        ...
        if (!cancelled) toast.error('Không tải được lịch sử thanh toán')
      } finally {
        if (!cancelled) setLoading(false)
      }
    }
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        <div className="flex flex-wrap gap-3 mb-3">
          <div className="relative flex-1 min-w-50">
            ...
            <input type="text" value={keyword} onChange={e => setKeyword(e.target.value)}
              ...
              placeholder="Biển số, mã phiên, tên khách..."
          ...
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            ...
            <option value="prepaid">Đã trả QR</option>
            <option value="surcharge">Có phụ trội</option>
          </select>
          ...
          <button onClick={handleSearch} disabled={loading}
            ...
            {loading ? <Loader2 size={14} className="animate-spin" /> : <Search size={14} />} Tìm
          </button>
        ...
        {filtered.length > 0 && (
          <div className="flex items-center gap-4 text-sm text-gray-500 border-t border-gray-100 pt-3">
            <span>{filtered.length} giao dịch</span>
            <span className="flex items-center gap-1">
              <TrendingUp size={13} className="text-green-500" />
              Tổng: <strong className="text-gray-800 ml-1">{formatVND(totalRevenue)}</strong>
            </span>
          </div>
        )}
      </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-50 text-gray-500 font-semibold border-b border-gray-100">
                <tr>
                  ...
                  <th className="py-3 px-4">Khách / Vị trí</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(s => (
                  <HistoryRow key={s.SessionID} session={s} onPrint={printInvoice} />
                ))}
              </tbody>
            </table>
          </div>
      ...
      <header className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <CreditCard size={24} className="text-blue-600" /> Thanh toán & Hóa đơn
          </h1>
          <p className="text-sm text-gray-500 mt-1">Thu phụ trội và xem lịch sử giao dịch</p>
        </div>
        <button onClick={() => navigate('/staff/checkout')}
          ...
          <Receipt size={15} /> Check-out xe mới
        </button>
      </header>
      ...
      <div className="flex bg-white rounded-xl border border-gray-200 shadow-sm p-1 gap-1 mb-6 w-fit">
        <button onClick={() => setTab(0)}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold transition-all ${
            tab === 0 ? 'bg-orange-500 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'
          }`}>
          <AlertCircle size={15} />
          Chờ thu phụ trội
          {surchargeCount > 0 && (
            <span className={`text-xs font-black px-2 py-0.5 rounded-full ${
              tab === 0 ? 'bg-white/20 text-white' : 'bg-orange-100 text-orange-600'
            }`}>{surchargeCount}</span>
          )}
        </button>
        <button onClick={() => setTab(1)}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold transition-all ${
            tab === 1 ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'
          }`}>
          <Clock size={15} /> Lịch sử thanh toán
        </button>
      </div>
