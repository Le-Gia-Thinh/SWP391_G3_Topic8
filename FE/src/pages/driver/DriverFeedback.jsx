...
const DriverFeedback = () => {
  ...
  return (
    <div className="animate-in fade-in duration-500 space-y-8">
      {/* Header */}
      <section>
        <h1 className="text-2xl font-bold text-gray-900">Đánh giá dịch vụ</h1>
        <p className="mt-1 text-sm text-gray-500">Chia sẻ trải nghiệm của bạn để chúng tôi cải thiện dịch vụ.</p>
      </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-8 shadow-2xl">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="text-xl font-bold text-gray-900">Đánh giá phiên gửi xe</h2>
                <p className="mt-1 text-sm text-gray-500">Biển số: {ratingForm.PlateNumber}</p>
              </div>
              <button onClick={closeRatingForm} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="rounded-xl bg-gray-50 p-4 mb-6 space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500 flex items-center gap-1.5"><MapPin size={14} /> Vị trí</span>
                <span className="font-bold text-gray-800">{ratingForm.SlotCode} — {ratingForm.BuildingName || '—'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500 flex items-center gap-1.5"><Clock size={14} /> Thời gian</span>
                <span className="font-bold text-gray-800">{formatDateTime(ratingForm.EntryTime)} → {formatDateTime(ratingForm.ExitTime)}</span>
              </div>
            </div>

            {/* Stars */}
            <div className="flex flex-col items-center mb-6">
              <p className="text-sm font-bold text-gray-700 mb-3">Bạn đánh giá bao nhiêu sao?</p>
            ...
            {/* Quick Tags */}
            <div className="mb-6">
              <p className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Gắn thẻ nhanh</p>
            ...
            {/* Comment */}
            <div className="mb-6">
              <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-gray-500">Bình luận (tùy chọn)</label>
              <textarea
                value={formComment}
                ...
                maxLength={500}
                rows={3}
                className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 transition focus:border-blue-500 focus:bg-white focus:outline-none resize-none"
              />
      ...
      <section>
        <div className="mb-5 flex items-center gap-2.5">
          ...
          <h2 className="text-lg font-bold text-gray-900">Phiên chưa đánh giá</h2>
        ...
        {unratedSessions.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50/50 py-12 text-center">
            <CheckCircle2 size={32} className="mx-auto mb-3 text-emerald-400" />
            <h3 className="text-base font-bold text-gray-900">Tuyệt vời!</h3>
            <p className="text-sm text-gray-500">Bạn đã đánh giá tất cả các phiên gửi xe.</p>
          </div>
        ) : (
            ...
            {unratedSessions.map((session) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900">{session.PlateNumber}</h3>
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-5 py-2.5 text-sm font-bold text-white shadow-md shadow-amber-200 transition hover:bg-amber-600 active:scale-95"
                >
                  <Star size={16} />
                  Đánh giá ngay
                </button>
      ...
      <section>
        <div className="mb-5 flex items-center gap-2.5">
          ...
          <h2 className="text-lg font-bold text-gray-900">Đánh giá đã gửi</h2>
        </div>

        {ratings.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50/50 py-12 text-center">
            <ThumbsUp size={32} className="mx-auto mb-3 text-gray-300" />
            <h3 className="text-base font-bold text-gray-900">Chưa có đánh giá</h3>
            <p className="text-sm text-gray-500">Đánh giá sau mỗi phiên gửi xe để giúp cải thiện dịch vụ.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {ratings.map((item) => (
              ...
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-900">{item.PlateNumber}</p>
                      <p className="text-xs text-gray-500">
                ...
                {item.Tags && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {item.Tags.split(', ').map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full bg-blue-50 px-2.5 py-0.5 text-[10px] font-bold text-blue-600"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                {item.Comment && (
                  <p className="mt-3 rounded-xl bg-gray-50 p-3 text-sm text-gray-600 italic">
                    "{item.Comment}"
                  </p>
                )}
              </div>
