    ...
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => {
        ...
        return (
          ...
          >
            <Star size={size} className={filled ? 'text-amber-400' : 'text-gray-300'} fill={filled ? 'currentColor' : 'none'} />
          </button>
        )
      })}
    </div>
...
const DriverFeedback = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (unratedResult.status === 'fulfilled') setUnratedSessions(unratedResult.value?.data || [])
      if (ratingsResult.status === 'fulfilled') setRatings(ratingsResult.value?.data || [])
    } catch {
      toast.error(t('driver.feedbackPage.loadError'))
    } finally {
      setLoading(false)
    }
  }, [t])
  ...
  const openRatingForm = (session) => { setRatingForm(session); setFormRating(0); setFormComment(''); setFormTags([]) }
  ...
  const handleSubmitRating = async () => {
    ...
    setSubmitting(true)
    try {
      // Convert key → label VN để BE lưu cố định, không phụ thuộc ngôn ngữ UI
      const tagsForBE = formTags.map(k => t(`driver.feedbackPage.tags.${k}`, { lng: 'vi' }))
      await driverApi.createRating({
        sessionId: ratingForm.SessionID,
        rating: formRating,
        comment: formComment.trim() || null,
        tags: tagsForBE.length > 0 ? tagsForBE : null
      })
      toast.success(t('driver.feedbackPage.modal.submitSuccess'))
      closeRatingForm()
      fetchData()
    } catch {
  ...
  return (
    <div className="animate-in fade-in duration-500 space-y-8">
      {/* Header */}
      <section>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('driver.feedbackPage.title')}</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('driver.feedbackPage.subtitle')}</p>
      </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-slate-800 p-8 shadow-2xl">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">{t('driver.feedbackPage.modal.title')}</h2>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('driver.feedbackPage.modal.plate')} {ratingForm.PlateNumber}</p>
              </div>
            ...
            <div className="rounded-xl bg-gray-50 dark:bg-slate-900/50 p-4 mb-6 space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500 dark:text-gray-400 flex items-center gap-1.5"><MapPin size={14} /> {t('driver.feedbackPage.modal.location')}</span>
                <span className="font-bold text-gray-800 dark:text-gray-200">{ratingForm.SlotCode} — {ratingForm.BuildingName || '—'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500 dark:text-gray-400 flex items-center gap-1.5"><Clock size={14} /> {t('driver.feedbackPage.modal.time')}</span>
            ...
            <div className="flex flex-col items-center mb-6">
              <p className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">{t('driver.feedbackPage.modal.ratingPrompt')}</p>
              <StarRating rating={formRating} size={36} interactive onChange={setFormRating} />
              <p className="mt-2 text-xs text-gray-400">
                {formRating === 0 && t('driver.feedbackPage.modal.ratingHint.select')}
                {formRating === 1 && t('driver.feedbackPage.modal.ratingHint.1')}
                {formRating === 2 && t('driver.feedbackPage.modal.ratingHint.2')}
                {formRating === 3 && t('driver.feedbackPage.modal.ratingHint.3')}
                {formRating === 4 && t('driver.feedbackPage.modal.ratingHint.4')}
                {formRating === 5 && t('driver.feedbackPage.modal.ratingHint.5')}
              </p>
            </div>

            {/* Quick Tags */}
            <div className="mb-6">
              <p className="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-3">{t('driver.feedbackPage.modal.tagsLabel')}</p>
              <div className="flex flex-wrap gap-2">
                {QUICK_TAG_KEYS.map((key) => {
                  const label = t(`driver.feedbackPage.tags.${key}`)
                  return (
                    <button
                      key={key}
                      type="button"
            ...
            {/* Comment */}
            <div className="mb-6">
              <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('driver.feedbackPage.modal.commentLabel')}</label>
              <textarea
                value={formComment}
                onChange={(e) => setFormComment(e.target.value)}
                placeholder={t('driver.feedbackPage.modal.commentPlaceholder')}
                maxLength={500}
                rows={3}
            ...
            <div className="flex justify-end gap-3">
              ...
              >
                {t('driver.feedbackPage.modal.cancel')}
              </button>
              <button
                onClick={handleSubmitRating}
                disabled={submitting || formRating < 1}
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 dark:bg-amber-600 px-5 py-2.5 text-sm font-bold text-white shadow-md shadow-amber-200 transition hover:bg-amber-600 active:scale-95"
              >
                {submitting ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />}
                {t('driver.feedbackPage.modal.submit')}
              </button>
            </div>
          </div>
        </div>
      ...
      <section>
        <div className="mb-5 flex items-center gap-2.5">
          ...
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">{t('driver.feedbackPage.unrated.title')}</h2>
        ...
        {unratedSessions.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-700 bg-gray-50/50 py-12 text-center">
            ...
            <h3 className="text-base font-bold text-gray-900 dark:text-white">{t('driver.feedbackPage.unrated.allDoneTitle')}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">{t('driver.feedbackPage.unrated.allDoneDesc')}</p>
          </div>
        ) : (
          <div className="space-y-3">
            {unratedSessions.map((session) => (
              ...
              >
                ...
                >
                  <Star size={16} />
                  {t('driver.feedbackPage.unrated.rateNow')}
                </button>
              </div>
            ))}
          </div>
      ...
      <section>
        <div className="mb-5 flex items-center gap-2.5">
          ...
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">{t('driver.feedbackPage.submitted.title')}</h2>
        </div>

        {ratings.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-700 bg-gray-50/50 py-12 text-center">
            <ThumbsUp size={32} className="mx-auto mb-3 text-gray-300" />
            <h3 className="text-base font-bold text-gray-900 dark:text-white">{t('driver.feedbackPage.submitted.emptyTitle')}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">{t('driver.feedbackPage.submitted.emptyDesc')}</p>
          </div>
        ) : (
                ...
                {item.Tags && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {item.Tags.split(', ').map((tag) => (
                      <span key={tag} className="rounded-full bg-blue-50 dark:bg-blue-900/20 px-2.5 py-0.5 text-[10px] font-bold text-blue-600 dark:text-blue-400">
                        {tag}
                      </span>
                    ))}
                  </div>
