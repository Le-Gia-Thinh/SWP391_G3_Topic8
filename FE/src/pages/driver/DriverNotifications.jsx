      ...
      <section className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            {unreadCount > 0 && (
              <span className="inline-flex items-center justify-center rounded-full bg-red-50 dark:bg-red-900/200 px-2.5 py-0.5 text-xs font-bold text-white">
                {unreadCount}
              </span>
            )}
          </div>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Theo dõi cập nhật về đặt chỗ, phiên gửi xe và thanh toán.</p>
        </div>
      ...
      <section className="space-y-3">
        {notifications.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-700 bg-gray-50/50 py-16 text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Chưa có thông báo</h3>
            ...
            return (
              <button
                ...
                className={`group flex w-full items-start gap-4 rounded-2xl border p-5 text-left transition-all hover:shadow-md hover:-translate-y-0.5 ${
                  notif.IsRead
                    ? 'border-gray-100 dark:border-slate-700/50 bg-white dark:bg-slate-800'
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className={`text-sm leading-snug ${
                      notif.IsRead ? 'font-semibold text-gray-700 dark:text-gray-300' : 'font-bold text-gray-900 dark:text-white'
                    }`}>
                      {notif.Title}
                    </h3>
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      {!notif.IsRead && (
                        <span className="h-2.5 w-2.5 rounded-full bg-blue-50 dark:bg-blue-900/200 animate-pulse" />
                      )}
                    </div>
                  </div>
                  <p className={`mt-1 text-sm leading-relaxed ${
                    notif.IsRead ? 'text-gray-500 dark:text-gray-400' : 'text-gray-600 dark:text-gray-400'
                  }`}>
                    {notif.Message}
                  </p>
