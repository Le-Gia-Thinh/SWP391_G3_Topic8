...
const Toggle = ({ checked, onChange, label, desc }) => (
  <div className="flex items-center justify-between py-3.5 border-b border-gray-50 dark:border-gray-700 last:border-0">
    <div>
      ...
      {desc && <p className="text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400 mt-0.5">{desc}</p>}
    </div>
    <button
      onClick={() => onChange(!checked)}
      className={`relative w-12 h-6 rounded-full transition-colors ${checked ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}
    >
      <span className={`absolute top-1 left-1 w-4 h-4 bg-white dark:bg-gray-800 rounded-full shadow transition-transform ${checked ? 'translate-x-6' : 'translate-x-0'}`} />
    </button>
  </div>
)
    ...
    <div className="flex gap-6 flex-col lg:flex-row animate-in fade-in">
      <div className="w-full lg:w-72 flex flex-col gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 flex flex-col items-center text-center">
          <div className="relative mb-4">
            <div className="flex h-24 w-24 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/50 text-3xl font-bold text-blue-600">
          ...
          <p className="text-lg font-black text-gray-900 dark:text-white">{formData?.fullName || user?.fullName || user?.FullName || t('navbar.defaultUser')}</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">{user?.email || user?.Email}</p>
          <span className="mt-2 px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 text-xs font-bold rounded-full border border-blue-200 dark:border-blue-800 uppercase">
      ...
      <div className="flex-1 flex flex-col gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
          <h3 className="text-base font-bold text-gray-800 dark:text-gray-100 mb-5">{t('userProfile.personalInfo')}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-semibold text-gray-400 dark:text-gray-500 dark:text-gray-400 uppercase flex items-center gap-1.5"><User size={16} className="text-blue-500" /> {t('userProfile.fullName')}</label>
              {editing ? <input value={formData?.fullName || ''} onChange={onChange('fullName')} className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 text-sm font-medium" /> : <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg">{formData?.fullName || t('userProfile.notUpdated')}</p>}
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              {editing ? <input value={formData?.phoneNumber || ''} onChange={onChange('phoneNumber')} className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 text-sm font-medium" /> : <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg">{formData?.phoneNumber || t('userProfile.notUpdated')}</p>}
            </div>
            <div className="space-y-1">
              <label className="text-xs font-semibold text-gray-400 dark:text-gray-500 dark:text-gray-400 uppercase flex items-center gap-1.5"><Calendar size={16} className="text-orange-500" /> {t('userProfile.dob')}</label>
              {editing ? <input type="date" value={formData?.dateOfBirth || ''} onChange={onChange('dateOfBirth')} className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 text-sm font-medium" /> : <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg">{formData?.dateOfBirth ? fmtDate(formData?.dateOfBirth) : t('userProfile.notUpdated')}</p>}
            </div>
            {isStaff && (
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-400 dark:text-gray-500 dark:text-gray-400 uppercase flex items-center gap-1.5"><MapPin size={16} className="text-red-500" /> {t('userProfile.gateLabel')}</label>
                <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg">Gate 01</p>
              </div>
            )}
            {isManager && (
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-400 dark:text-gray-500 dark:text-gray-400 uppercase flex items-center gap-1.5"><Building size={16} className="text-orange-500" /> {t('userProfile.branch')}</label>
                <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg">SmartPark District 1</p>
              </div>
            )}
            {isDriver && (
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-400 dark:text-gray-500 dark:text-gray-400 uppercase flex items-center gap-1.5"><CreditCard size={16} className="text-teal-500" /> {t('userProfile.balance')}</label>
                <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2"><Activity size={18} className="text-blue-500" /> {t('userProfile.recentActivity')}</h3>
              </div>
              <div className="space-y-3">
                {recentActivity?.length === 0 ? (
                  <p className="text-sm text-gray-500 dark:text-gray-400 italic">{t('userProfile.noActivity')}</p>
                ) : (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2"><CreditCard size={18} className="text-emerald-500" /> {t('userProfile.paymentHistory')}</h3>
              </div>
              <div className="space-y-3">
                {recentPayments?.length === 0 ? (
                  <p className="text-sm text-gray-500 dark:text-gray-400 italic">{t('userProfile.noPayment')}</p>
                ) : (
                  recentPayments?.map((pay, i) => (
                    <div key={pay.PaymentID || i} className="flex justify-between items-center p-3 rounded-xl bg-gray-50 dark:bg-gray-700 border border-gray-100 dark:border-gray-700">
                      <div>
                        <p className="text-sm font-bold text-gray-800 dark:text-gray-100">{new Intl.NumberFormat('vi-VN').format(pay.Amount || 0)} VNĐ</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{fmtDate(pay.PaymentTime)} {fmtTime(pay.PaymentTime)}</p>
                      </div>
    ...
    <div className="flex gap-6 flex-col lg:flex-row animate-in fade-in">
      <div className="flex-1 space-y-5">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
          ...
          <div className="mb-5">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-300 mb-3 mt-4">{t('userProfile.settings.colorTheme')}</p>
            <div className="flex gap-3">
              {[
                ...
                <button
                  ...
                  className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${(id === 'dark' && theme === 'dark') || (id === 'light' && theme === 'light') ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' : 'border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400'}`}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
          <h3 className="text-base font-bold text-gray-800 dark:text-gray-100 mb-1 flex items-center gap-2"><Bell size={18} className="text-purple-500" /> {t('userProfile.settings.notifications')}</h3>
...
const SecurityContent = () => {
  ...
  return (
    <div className="flex gap-6 flex-col lg:flex-row animate-in fade-in">
      <div className="flex-1 space-y-5">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
          ...
          <form onSubmit={handleChangePassword} className="space-y-4">
            {hasPassword && (
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">{t('userProfile.security.currentPassword')}</label>
                <div className="relative">
                  <input type={showOld ? 'text' : 'password'} value={oldPw} onChange={e => setOldPw(e.target.value)} className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl" />
                  <button type="button" onClick={() => setShowOld(!showOld)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 dark:text-gray-400">{showOld ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                </div>
              </div>
            )}
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">{t('userProfile.security.newPassword')}</label>
              <div className="relative">
                <input type={showNew ? 'text' : 'password'} value={newPw} onChange={e => setNewPw(e.target.value)} className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl" />
                <button type="button" onClick={() => setShowNew(!showNew)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 dark:text-gray-400">{showNew ? <EyeOff size={18} /> : <Eye size={18} />}</button>
              </div>
            </div>
      ...
      <div className="w-full lg:w-80 flex flex-col gap-5">
        <div className="bg-red-50 dark:bg-red-900/20 rounded-2xl border border-red-100 dark:border-red-800/50 p-5">
          <h3 className="text-sm font-bold text-red-800 dark:text-red-400 mb-3 flex items-center gap-2"><AlertTriangle size={16} /> {t('userProfile.security.dangerZone')}</h3>
  ...
  return (
    <div className="flex flex-col h-full space-y-6 animate-in fade-in">
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{TAB_TITLES[activeTab]}</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('userProfile.subtitle')}</p>
        </div>
            ...
            {PROFILE_TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition-all text-left ${active ? 'bg-blue-600 text-white shadow-md shadow-blue-100 dark:shadow-none' : 'text-gray-600 hover:bg-blue-50 dark:bg-blue-900/30 hover:text-blue-600'}`}
                >
                  <Icon size={18} />
                  <span>{tab.label}</span>
